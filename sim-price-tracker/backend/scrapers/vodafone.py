        ...
        for url in self.urls:
            try:
                ...
                if html and len(html) > 5000:
                    ...
                    plans.extend(found)
                    logger.info(f"Vodafone: Found {len(found)} from {url}")
            except Exception as e:
                logger.error(f"Vodafone error: {e}")
        ...
        for ps in prices:
            ...
            for d in data_list:
                ...
                if 1 <= v <= 500:
                    ...
                    data_list.remove(d)
                    break
            name = f"Vodafone {gb}GB" if gb else "Vodafone Plan"
            plans.append(ScrapedPlan(name=name, price=price, data_gb=gb, data_unlimited=(gb is None), contract_months=24, url=url, is_5g=has_5g, external_id=f"vodafone_{price}_{gb or 0}"))
        return plans
    ...
    def _walk_json(self, data, url: str, depth=0) -> List[ScrapedPlan]:
        ...
        if isinstance(data, dict):
            ...
            if price and 0 < price < 200:
                ...
                name = data.get('name', f"Vodafone {gb}GB" if gb else "Vodafone Unlimited" if unlim else "Vodafone Plan")
                plans.append(ScrapedPlan(name=name, price=price, data_gb=gb if not unlim else None, data_unlimited=unlim, contract_months=24, url=url, is_5g=is5g, external_id=f"vodafone_json_{price}_{gb or 0}"))
