...
class TalkmobileScraper(BaseScraper):
    ...
    urls = ["https://www.talkmobile.co.uk/sim-only", "https://www.talkmobile.co.uk/"]
            ...
            try:
                try:
                    ...
                    if len(html) > 5000:
                        ...
                        if found:
                            plans.extend(found)
                            logger.info(f"Talkmobile: Found {len(found)} via httpx")
                ...
                if len(plans) < 3:
                    ...
                    if html and len(html) > 5000:
                        found = self._parse(html, url)
                        plans.extend(found)
                        logger.info(f"Talkmobile: Found {len(found)} via Playwright")
            except Exception as e:
                logger.error(f"Talkmobile error: {e}")
    ...
    def _parse(self, html: str, url: str) -> List[ScrapedPlan]:
        ...
        for ps in prices:
            ...
            name = f"Talkmobile {gb}GB" if gb else "Talkmobile Plan"
            ext_id = f"talkmobile_{price}_{gb or 0}"
    ...
    def _walk_json(self, data, url: str, depth=0) -> List[ScrapedPlan]:
        ...
        if isinstance(data, dict):
            ...
            if price and 0 < price < 200:
                ...
                pname = data.get('name', f"Talkmobile {gb}GB" if gb else "Talkmobile Unlimited" if unlim else "Talkmobile Plan")
                ext_id = f"talkmobile_json_{price}_{gb or 0}"
