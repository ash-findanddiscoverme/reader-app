...
class LycaMobileScraper(BaseScraper):
    ...
    provider_slug = "lyca_mobile"
    provider_type = "mvno"
    base_url = "https://www.lycamobile.co.uk/en/bundle/"
    urls = ["https://www.lycamobile.co.uk/en/bundle/", "https://www.lycamobile.co.uk/"]
            ...
            try:
                try:
                    ...
                    if len(html) > 5000:
                        ...
                        if found:
                            plans.extend(found)
                            logger.info(f"Lyca Mobile: Found {len(found)} via httpx")
                ...
                if len(plans) < 3:
                    ...
                    if html and len(html) > 5000:
                        found = self._parse(html, url)
                        plans.extend(found)
                        logger.info(f"Lyca Mobile: Found {len(found)} via Playwright")
            except Exception as e:
                logger.error(f"Lyca Mobile error: {e}")
    ...
    def _parse(self, html: str, url: str) -> List[ScrapedPlan]:
        ...
        for ps in prices:
            ...
            name = f"Lyca Mobile {gb}GB" if gb else "Lyca Mobile Plan"
            ext_id = f"lyca_mobile_{price}_{gb or 0}"
    ...
    def _walk_json(self, data, url: str, depth=0) -> List[ScrapedPlan]:
        ...
        if isinstance(data, dict):
            ...
            if price and 0 < price < 200:
                ...
                pname = data.get('name', f"Lyca Mobile {gb}GB" if gb else "Lyca Mobile Unlimited" if unlim else "Lyca Mobile Plan")
                ext_id = f"lyca_mobile_json_{price}_{gb or 0}"
