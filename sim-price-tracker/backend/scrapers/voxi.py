...
class VOXIScraper(BaseScraper):
    ...
    urls = ["https://www.voxi.co.uk/plans", "https://www.voxi.co.uk/"]
            ...
            try:
                try:
                    ...
                    if len(html) > 5000:
                        ...
                        if found:
                            plans.extend(found)
                            logger.info(f"VOXI: Found {len(found)} via httpx")
                ...
                if len(plans) < 3:
                    ...
                    if html and len(html) > 5000:
                        found = self._parse(html, url)
                        plans.extend(found)
                        logger.info(f"VOXI: Found {len(found)} via Playwright")
            except Exception as e:
                logger.error(f"VOXI error: {e}")
    ...
    def _parse(self, html: str, url: str) -> List[ScrapedPlan]:
        ...
        for ps in prices:
            ...
            name = f"VOXI {gb}GB" if gb else "VOXI Plan"
            ext_id = f"voxi_{price}_{gb or 0}"
    ...
    def _walk_json(self, data, url: str, depth=0) -> List[ScrapedPlan]:
        ...
        if isinstance(data, dict):
            ...
            if price and 0 < price < 200:
                ...
                pname = data.get('name', f"VOXI {gb}GB" if gb else "VOXI Unlimited" if unlim else "VOXI Plan")
                ext_id = f"voxi_json_{price}_{gb or 0}"
