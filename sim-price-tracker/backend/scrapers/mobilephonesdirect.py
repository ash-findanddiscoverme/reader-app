...
class MobilePhonesDirectScraper(BaseScraper):
    ...
    base_url = "https://www.mobilephonesdirect.co.uk/sim-only-deals"
            ...
            try:
                try:
                    ...
                    if len(html) > 5000:
                        ...
                        if found:
                            plans.extend(found)
                            logger.info(f"Mobile Phones Direct: Found {len(found)} via httpx")
                ...
                if len(plans) < 3:
                    ...
                    if html and len(html) > 5000:
                        found = self._parse(html, url)
                        plans.extend(found)
                        logger.info(f"Mobile Phones Direct: Found {len(found)} via Playwright")
            except Exception as e:
                logger.error(f"Mobile Phones Direct error: {e}")
    ...
    def _parse(self, html: str, url: str) -> List[ScrapedPlan]:
        ...
        for ps in prices:
            ...
            name = f"Mobile Phones Direct {gb}GB" if gb else "Mobile Phones Direct Plan"
            ext_id = f"mobilephonesdirect_{price}_{gb or 0}"
    ...
    def _walk_json(self, data, url: str, depth=0) -> List[ScrapedPlan]:
        ...
        if isinstance(data, dict):
            ...
            if price and 0 < price < 200:
                ...
                pname = data.get('name', f"Mobile Phones Direct {gb}GB" if gb else "Mobile Phones Direct Unlimited" if unlim else "Mobile Phones Direct Plan")
                ext_id = f"mobilephonesdirect_json_{price}_{gb or 0}"
